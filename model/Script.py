...
# Display the original image and the image with extracted unmarked features (bounding boxes)
...
# Show the final image with green contours (drawings) and red bounding boxes (unmarked areas)
plt.subplot(1, 2, 2)
plt.imshow(cv2.cvtColor(final_output_image, cv2.COLOR_BGR2RGB))
plt.title("Drawings + Unmarked Features (Bounding Boxes)")
plt.axis('off')
